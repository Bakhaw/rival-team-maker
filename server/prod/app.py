...
sql_con = conn = sqlite3.connect("rival_team_maker.db", check_same_thread=False)
...
def get_best_community(com_id):
    ...
    sql = """   SELECT oc.com_id, s.mapId, CAST(SUM(s.sum_score) AS FLOAT) / SUM(nb_played) score
                ...
                INNER JOIN dim_com_clusters mc ON mc.com_id = {com_id} AND mc.cluster_hour_con=oc.cluster_hour_con AND mc.level_cat=oc.level_cat
...
def get_search_by_id(result_id):
    ...
    res = get_community__nostring(communities[result_id % len(communities)])
    res['me'] = get_community__nostring(base_community)
...
def get_community__nostring(community_id):
    desc = {
        'community_id':0
        , 'name': ''
        , 'nb_player':0
...
def get_connexions(community_id):
    ...
    for row in cursor.fetchall():
        ...
        if r.nb_connexions > max_nb_con:
            max_nb_con = r.nb_connexions
        res.append({'x':r.week_day + r.hour, 'y':r.nb_connexions})

    for i, kv in enumerate(res):
        res[i]['y'] = float(kv['y']) / float(max_nb_con)
    return res
